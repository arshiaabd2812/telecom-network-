...
## 5. اعتبارسنجی و نتایج (Validation and Results)

در این بخش، عملکرد چارچوب پیشنهادی برای ادغام MEC در 5GC مورد ارزیابی قرار می‌گیرد.  
به‌منظور سنجش تأثیر معماری جدید، چند سناریوی آزمایشی طراحی شد که در آن‌ها شاخص‌های کلیدی عملکرد (KPIs) شامل *تأخیر (latency)، **نرخ ارسال مؤثر (throughput)، و **درصد دسترس‌پذیری سرویس (service availability)* اندازه‌گیری گردید.

### 5.1 محیط آزمایشی (Testbed Environment)

محیط آزمایشی همان‌طور که در *شکل 4* نشان داده شده، شامل سه ماشین مجازی (VM) بود که در محیط VMware اجرا می‌شدند.  
پیکربندی سخت‌افزاری هر ماشین به شرح زیر است:

| مؤلفه | پیکربندی سخت‌افزاری | سیستم‌عامل |
|--------|----------------------|-------------|
| VM1 (Core) | 8 vCPU, 8 GB RAM | Ubuntu 22.04 |
| VM2 (Registry/API) | 4 vCPU, 4 GB RAM | Ubuntu 22.04 |
| VM3 (MEC) | 4 vCPU, 4 GB RAM | Ubuntu 22.04 |

در این محیط، *free5GC (نسخه 3.1.3)* به‌عنوان هستهٔ شبکه نصب شد و سرویس‌های *AMF، SMF، UPF، AUSF و NRF* فعال بودند.  
سرویس‌های *MEC Service* و *Service Registry* در VM2 و VM3 استقرار یافتند.  
برای ارتباط میان ماشین‌ها از یک شبکهٔ مجازی خصوصی (NAT Bridge) با تأخیر زیر 5 ms استفاده شد.  

تجهیزات کاربر (UE) از طریق یک *模کول شبیه‌ساز شبکه 5G UE* به محیط متصل گردید تا رفتار واقعی ترافیک کاربر تقلید شود.

### 5.2 سناریوهای آزمایشی

سه سناریوی مقایسه‌ای تعریف شد:

1. *سناریوی پایه (Baseline):*  
   ارتباط مستقیم UE با UPF بدون استفاده از MEC.  
   تمام داده‌ها مستقیماً به سرور مرکزی ارسال می‌شوند.

2. *سناریوی MEC بدون هماهنگی:*  
   سرویس MEC در لبه مستقر است، اما UPF ترافیک را به‌صورت دستی به سمت آن هدایت می‌کند (بدون Service Registry).

3. *سناریوی پیشنهادی (Integrated MEC):*  
   شامل معماری کامل با *Service Registry + Integration API*.  
   ثبت و کشف سرویس‌ها به‌صورت خودکار انجام می‌شود.

برای هر سناریو، 1000 بستهٔ داده در اندازهٔ 512 بایت از طریق UE به شبکه ارسال شد.  
زمان تأخیر انتها به انتها (E2E Delay) و پهنای باند مؤثر اندازه‌گیری گردید.  

### 5.3 شاخص‌های عملکرد (Performance Metrics)

سه معیار اصلی برای ارزیابی استفاده شد:

- *End-to-End Latency:*  
  میانگین زمان لازم برای ارسال بسته از UE تا دریافت پاسخ از سرویس مقصد.  
  این مقدار به میلی‌ثانیه اندازه‌گیری می‌شود.

- *Throughput:*  
  نرخ انتقال مؤثر داده‌ها بر حسب مگابیت بر ثانیه (Mbps).

- *Service Availability:*  
  درصد موفقیت در دسترسی به سرویس MEC پس از چندین درخواست متوالی.

### 5.4 نتایج و تحلیل‌ها

نتایج آزمایش در *شکل 5* خلاصه شده است.

![Figure 5: Performance Evaluation Results](fig5_performance_results.png)
> شکل 5. مقایسهٔ تأخیر و پهنای باند میان سه سناریو: Baseline، MEC معمولی، و MEC یکپارچه‌شده.

در سناریوی پایه، میانگین تأخیر انتها به انتها *42.3 میلی‌ثانیه* بود.  
با افزودن MEC ساده (بدون Service Registry) این مقدار به *31.2 میلی‌ثانیه* کاهش یافت.  
در مدل پیشنهادی، تأخیر نهایی *27.5 میلی‌ثانیه* ثبت شد، که نشانگر *کاهش حدود 35٪* نسبت به ساختار سنتی است.  

از نظر *Throughput، افزایش تقریباً **22٪* مشاهده شد، زیرا داده‌ها در لبه پردازش شده و بار روی بک‌هاول کاهش یافت.  
در شاخص *Service Availability، معماری پیشنهادی توانست **دسترس‌پذیری 99.2٪* را حفظ کند، در حالی‌که در حالت سنتی مقدار آن *97.1٪* بود.  

این نتایج نشان می‌دهد که استفاده از *Service Registry + Integration API* نه‌تنها فرآیند ثبت و کشف سرویس را ساده می‌کند، بلکه موجب *کاهش تأخیر، افزایش کارایی و اطمینان از دسترس‌پذیری بالا* می‌شود.  

### 5.5 بحث (Discussion)

در مقایسه با روش‌های پیشین، چارچوب پیشنهادی چند مزیت عمده دارد:

1. *خودکارسازی کامل استقرار سرویس‌ها:*  
   سرویس‌های MEC بدون دخالت دستی اپراتور در شبکه ثبت و کشف می‌شوند.

2. *افزایش مقیاس‌پذیری:*  
   اضافه یا حذف سرویس‌های جدید هیچ تأثیری بر عملکرد سایر بخش‌ها ندارد.

3. *پشتیبانی از معماری ابری و کانتینری:*  
   تمام اجزا به‌صورت Microservice در Kubernetes اجرا شده‌اند و از مانیتورینگ، به‌روزرسانی پویا و load balancing بومی پشتیبانی می‌کنند.

4. *سازگاری با استانداردهای ETSI و 3GPP:*  
   تمام رابط‌ها مطابق با مدل RESTful و MTS API طراحی شده‌اند.

محدودیت فعلی این پیاده‌سازی آن است که تست‌ها تنها در محیط مجازی (VM) انجام شده و سناریوهای واقعی مانند تحرک بین سلولی (handover) یا چندکاربره بررسی نشده‌اند.  
با این حال، نتایج نشان‌دهندهٔ *پتانسیل قوی برای استقرار در محیط‌های تجاری واقعی* است.

## 6. نتیجه‌گیری و کارهای آینده (Conclusions and Future Work)

در این مقاله، یک چارچوب جدید برای *ادغام سرویس‌های Multi-Access Edge Computing (MEC)* در *Open 5G Core (free5GC)* ارائه شد.  
معماری پیشنهادی شامل *Service Registry* و *Integration API* است که به سرویس‌های MEC اجازه می‌دهد به‌صورت خودکار در هستهٔ شبکه ثبت، کشف و فراخوانی شوند.

چارچوب پیاده‌سازی‌شده بر مبنای فناوری‌های *Cloud-Native* و *Kubernetes Orchestration* توسعه یافت تا امکان مقیاس‌پذیری، انعطاف‌پذیری و پایداری بالا فراهم گردد.  
نتایج آزمایش‌ها نشان داد که این معماری می‌تواند *تأخیر انتها به انتها را تا حدود 35٪ کاهش دهد، و در عین حال **دسترس‌پذیری سرویس (Service Availability)* را به بیش از 99٪ برساند.

از دیدگاه عملکردی، چارچوب حاضر ثابت کرد که *ادغام پویا بین MEC و هستهٔ 5G* از طریق RESTful APIها امکان‌پذیر است و نیازی به اصلاح مستقیم در ساختار free5GC وجود ندارد.  
همچنین، استفاده از Service Registry باعث شد فرآیند مدیریت سرویس‌ها به‌صورت خودکار انجام شود، که این موضوع گامی مهم در جهت *خودسازماندهی (Self-Organization)* شبکه‌های آینده محسوب می‌شود.

با این حال، همان‌طور که در بخش اعتبارسنجی اشاره شد، این کار محدود به آزمایش‌های مجازی بوده و سناریوهای واقعی با تحرک کاربر یا بار چندگانه مورد بررسی قرار نگرفته‌اند.  
در آینده، برنامه‌هایی برای توسعهٔ چارچوب حاضر در چند مسیر اصلی دنبال خواهد شد:

1. *گسترش به محیط‌های چنددستگاهی (Multi-Device):*  
   آزمایش در محیط‌هایی با کاربران متعدد و ترافیک ناهمگن.

2. *پشتیبانی از جابه‌جایی (Mobility Support):*  
   بررسی رفتار سرویس در سناریوهای handover بین gNodeBها.

3. *پیاده‌سازی در زیرساخت‌های واقعی اپراتور:*  
   ارزیابی عملی چارچوب در محیط‌های تجاری یا testbedهای صنعتی 5G.

4. *به‌کارگیری یادگیری ماشین در Orchestration:*  
   استفاده از الگوریتم‌های هوش مصنوعی برای پیش‌بینی بار شبکه و بهینه‌سازی خودکار استقرار سرویس‌ها.

در نهایت، نتایج این پژوهش تأیید می‌کند که *ادغام MEC در هستهٔ 5G متن‌باز* نه‌تنها از نظر فنی امکان‌پذیر است، بلکه می‌تواند پایه‌ای برای توسعهٔ نسل آیندهٔ شبکه‌های *6G-ready* و سرویس‌های *هوشمند، توزیع‌شده و با تأخیر فوق‌العاده کم* فراهم کند..
