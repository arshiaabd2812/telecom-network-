...
- ## 2. کدهای قطبی (Polar Codes)

کدهای قطبی (Polar Codes) نخستین خانواده از کدهای بلوکی هستند که به‌صورت *اثبات‌شده از نظر نظری قادر به دستیابی به ظرفیت کانال شانون* می‌باشند.  
این کدها نخستین بار توسط *Arıkan* در سال 2009 معرفی شدند و بر پایهٔ پدیده‌ای به نام *قطبش کانال (Channel Polarization)* عمل می‌کنند.  
ایدهٔ اصلی آن است که با تبدیل چندین کانال یکسان و مستقل به مجموعه‌ای از کانال‌های جدید، برخی از آن‌ها *کاملاً قابل اطمینان* و برخی دیگر *کاملاً غیرقابل اطمینان* می‌شوند.  
در عمل، بیت‌های اطلاعات تنها از طریق کانال‌های قابل اطمینان ارسال می‌شوند، در حالی‌که بیت‌های مربوط به کانال‌های ضعیف، به‌صورت ثابت (مثلاً با مقدار صفر) نگهداری می‌گردند.

در سیستم پیشنهادی حاضر، کدهای قطبی برای *کاهش نرخ خطای بیت (BER)* و *افزایش قابلیت اطمینان انتقال در کانال‌های صوتی زیرآب (UWA)* به کار گرفته شده‌اند.  
کدگذاری قطبی از یک ساختار ماتریسی مبتنی بر تبدیل *کرنل (Kernel Transform)* استفاده می‌کند که به‌صورت زیر تعریف می‌شود:
...
برای بلوکی با طول \( N = 2^n \)، ماتریس تبدیل کلی از طریق ضرب تکراری کرنل به‌صورت زیر به‌دست می‌آید:

\[
G_N = F^{\otimes n}
\]

که در آن \( \otimes \) نشان‌دهندهٔ ضرب کرونکر (Kronecker Power) است.  
فرآیند کدگذاری با ضرب بردار ورودی \( u_N \) در ماتریس \( G_N \) انجام می‌شود:

\[
x_N = u_N \cdot G_N
\]

در اینجا \( u_N \) شامل *بیت‌های اطلاعاتی* و *بیت‌های منجمد (Frozen Bits)* است.  
موقعیت بیت‌های منجمد بر اساس احتمال خطای کانال‌های فرعی تعیین می‌شود.  
این احتمال معمولاً با استفاده از معیارهایی مانند *احتمال خروجی لوژیت (Log-Likelihood Ratio - LLR)* یا تقریب‌های تحلیل متقابل تخمین زده می‌شود.

در فرآیند *رمزگشایی (Decoding)، از الگوریتم **Successive Cancellation (SC)* استفاده می‌شود.  
در این روش، تصمیم‌گیری بر اساس احتمال شرطی هر بیت با توجه به بیت‌های قبلاً رمزگشایی‌شده انجام می‌گیرد.  
الگوریتم SC از نظر پیچیدگی محاسباتی دارای مرتبه \( O(N \log N) \) است که آن را برای کاربردهای بی‌سیم و کانال‌های نویزی مناسب می‌سازد.  
با این حال، برای بهبود عملکرد در نرخ‌های پایین و طول بلوک‌های کوچک، از نسخه‌های پیشرفته‌تر مانند *Successive Cancellation List (SCL)* و *CRC-Aided SCL (CA-SCL)* نیز می‌توان بهره برد.

در این مقاله، از *کدهای قطبی استاندارد با الگوریتم رمزگشایی SC* برای بررسی تأثیر بهبود عملکرد BER در سیستم‌های مدولاسیون آشوبی DCSK و QCSK استفاده شده است.  
به‌کارگیری این کدها باعث کاهش محسوس خطاهای بیت در کانال‌های زیرآب با توزیع نویز t می‌گردد، جایی که شرایط نویزی شدیدتر از توزیع گاوسی معمولی است.

به‌صورت خلاصه، مزایای اصلی استفاده از کدهای قطبی در این سیستم عبارت‌اند از:  
1. دستیابی به ظرفیت کانال در حد تئوری شانون؛  
2. پیچیدگی پایین رمزگشایی در مقایسه با کدهای LDPC و Turbo؛  
3. سازگاری با مدولاسیون آشوبی و کانال‌های دارای نویز ضربه‌ای؛  
4. امکان پیاده‌سازی آسان در سیستم‌های بلادرنگ و کم‌مصرف مانند سامانه‌های زیرآبی.

---

در بخش بعدی (*بخش 3)، **مدل کانال UWA* مورد استفاده در این پژوهش توضیح داده می‌شود.
